 ...
 * on http://www.javagaming.org forum and adapted to be more generic. 
 * Antialiasing and texture filling capability added.
 * 
...
public final class FloodFill {
  ...
  public void fill(int x, int y, Color color, Color patternColor, int patternWidth, int patternHeight) {
    ...
    this.maskColor = 0x735108C2;     // random
    if (this.maskColor == this.startColor) {
      this.maskColor = 0x3A3B55FD;   // random
    }
        ...
        while (x <= right) {
          if (maskImage[yOff + x] == startColor) {
            ...
            if (antialiased) {
              ...
              if (antialiasedColor != maskColor) {
                antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
                antialiasedColor += (fillColor(x, y - 1) >> 1) & 0x7F7F7F7F;      
  ...
  private int floodFill(int x, int y) { 
    ...
    int left = x;
    do {
      image[yOff + left] = fillColor(left, y);
    ...
    if ((antialiased) && (left >= 0)) {
      ...
      if (antialiasedColor != maskColor) {
        antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
        antialiasedColor += (fillColor(left, y) >> 1) & 0x7F7F7F7F;      
    ...
    int right = x;
    do {
      image[yOff + right] = fillColor(right, y);
    ...
    if ((antialiased) && (right < width)) {
      ...
      if (antialiasedColor != maskColor) {
        antialiasedColor = (antialiasedColor >> 1) & 0x7F7F7F7F;
        antialiasedColor += (fillColor(right, y) >> 1) & 0x7F7F7F7F;      
   ...
   */
  private int fillColor(int x, int y) {
    x /= this.patternWidth;
