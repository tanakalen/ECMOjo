...
/**
 * The clock-type transition.
 *
 * @author   kinwang
 * @since    January 7, 2009
 */
public class ClockTransition implements Transition {
  ...
  public void update() {
    ...
    int radius = 500;
    int angle = (int) (Math.pow(2, elapsed / 50) * 360.0 / 2000.0);
    if (angle > 360) {
      angle = 360;
      completed = true;
    }
    ...
    g2.drawArc(center.x - radius, center.y - radius, 2 * radius, 2 * radius, 90, -angle);
    g2.setComposite(AlphaComposite.Clear);
    g2.fillArc(center.x - radius, center.y - radius, 2 * radius, 2 * radius, 90, -angle);
    g2.dispose();
  }
