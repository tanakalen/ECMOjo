...
/**
 * The Perlin Noise function by Ken Perlin. http://mrl.nyu.edu/~perlin/noise/
 * 
 * @author king
...
public final class PerlinNoise {
  ...
  /**
   * Static initializer constructs p from permutations.
   */
  static {
    for (int i = 0; i < 256; i++) {
  ...
  public static double noise(double x, double y, double z) {
    ...
    return lerp(w, lerp(v, lerp(u, grad(P[iAA], x, y, z), grad(P[iBA], x - 1, y, z)), 
                           lerp(u, grad(P[iAB], x, y - 1, z), grad(P[iBB], x - 1, y - 1, z))),
                   lerp(v, lerp(u, grad(P[iAA + 1], x, y, z - 1), grad(P[iBA + 1], x - 1, y, z - 1)), 
                           lerp(u, grad(P[iAB + 1], x, y - 1, z - 1), grad(P[iBB + 1], x - 1, y - 1, z - 1))));
  }
